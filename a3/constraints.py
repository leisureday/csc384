...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        domainSize = len(qi._dom)
        satisfyingAssignments = []
        for vi in range(1, domainSize+1):
            for vj in range(1, domainSize+1):
                # check both column constraint and diagonal constraint
                if (vi!=vj) and (abs(vi-vj)!=abs(i-j)):
                    satisfyingAssignments.append([vi, vj])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        return self._lb <= self.scope().count(self._required) <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        if var not in self.scope():
            return True
        remainingVars = self.scope()
        remainingVars.remove(var)  
        count = 0
        if val == self._required:
            count = 1
        for remainingVar in remainingVars:
            if self._required in remainingVar.curDomain():
                count += 1
        return count >= self._lb
